...
namespace Sewers
{
	...
	{
		// Used for to make extraction easy, via the >> operator
		ifstream ifs;
		...
		{		
			if(current_type == "SWITCH")
			{
				...
				_switches[_num_switches-1].set_width(SWITCH_WIDTH);
				_switches[_num_switches-1].set_height(SWITCH_HEIGHT);
				_switches[_num_switches-1].set_output_value(output_value);
				// Connect the object to the rest of the circuit
				connect_object(_switches[_num_switches-1]);
			}
			...
			else 
			{
				...
				_gates[_num_gates-1].set_width(GATE_WIDTH);
				_gates[_num_gates-1].set_height(GATE_HEIGHT);
				// Connect the object to the rest of the circuit
				connect_object(_gates[_num_gates-1]);
			}			
		}
	...
	// PRECONDITIONS: none
	// POSTCONITIONS: Truth table for the current room has been drawn
    ...
    void GameEngine::attempt_avatar_move(string direction)
    {
		...
		if((!_entrance.is_ladder() &&
		   ...
		   (proposed_position.left() > _entrance.right() &&
			...
			// If so, allow the avatar to move
